-- ═══════════════════════════════════════════════════════════════════════════
-- FINAL FIX: create_queue_ticket Function with Correct Data Types
-- ═══════════════════════════════════════════════════════════════════════════
-- Purpose: Fix data type mismatch - citizen_id should be BIGINT not UUID
-- Issue: Previous migrations incorrectly used UUID for citizen_id
-- Fix: Use BIGINT to match queue_tickets table schema
-- ═══════════════════════════════════════════════════════════════════════════
...
RETURNS TABLE (
    id BIGINT,
    citizen_id BIGINT,  -- FIXED: Changed from UUID to BIGINT to match table schema
...
BEGIN
    ...
    -- Get the authenticated user's citizen_id (BIGINT from citizens.id)
...
COMMENT ON FUNCTION create_queue_ticket(TEXT, TEXT, TEXT, TEXT, TEXT) IS 
  'Creates a new queue ticket with validation and daily limit check. Uses BIGINT for citizen_id to match table schema. Includes input sanitization and whitespace validation.';
