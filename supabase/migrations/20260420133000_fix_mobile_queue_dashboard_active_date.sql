-- Fix mobile queue dashboard state resolution by avoiding strict current_date filtering.
-- This keeps queue state accurate even when active queue tickets are on a different server-local date.
...
begin
  ...
  -- Resolve the latest active queue ticket across dates.
  select
    q.id,
    q.service_key,
    q.service_label,
