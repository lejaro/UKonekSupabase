-- Fix: Restore the 16-column signature of get_my_queue_dashboard to return is_on_call,
-- waiting_count (the real count of patients ahead in the queue), and citizen details.
...
CREATE OR REPLACE FUNCTION public.get_my_queue_dashboard()
 RETURNS TABLE(
   ...
   -- New fields for QR code autofill
...
begin
  ...
  -- Find the most recent active ticket for today (standardized to Manila Date, status: waiting, on_call, serving)
  select
    q.id,
    q.service_key,
    q.service_label,
  ...
  -- The real count of patients ahead in the queue is (v_waiting_ahead + v_on_call_ahead)
