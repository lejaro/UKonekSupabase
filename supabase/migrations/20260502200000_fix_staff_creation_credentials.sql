-- Fix create_staff_account_admin to ensure credentials are valid upon creation.
-- This aligns manual auth.users/identities insertion with GoTrue's expectations.
...
begin
  ...
  -- Insert into auth.users with all necessary fields to ensure "confirmed" status
  ...
  -- Important: for email provider, identity_data should include email_verified
  insert into auth.identities (
    id,
    user_id,
    identity_data,
  ...
  values (
    v_auth_user_id, -- Using user_id as identity_id is common for primary email identity
    v_auth_user_id,
    jsonb_build_object(
    ...
    'email',
    v_email, -- provider_id for email provider is usually the email itself
    now(),
    now(),
    now()
  );
