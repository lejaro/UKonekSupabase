-- Drop existing function first to avoid return type mismatch error
...
begin
  ...
  where q.queue_date = v_queue_date
    ...
    and lower(trim(coalesce(q.status, ''))) in ('serving', 'on_call');
  ...
  if lower(trim(coalesce(v_status, ''))) in ('serving', 'on_call') then
